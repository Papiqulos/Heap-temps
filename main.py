...
def running_median(n):
    ...
    for i in range(n):
        ...
        # Getting the temperatures from a list where the temperatures have already been generated
        # coord = coords[i]
        # temp = temps[i]

        # Check if the coordinate is already in either heap and update the temperature if so
        if max_h.isInMinHeap(coord):
            # max_h.changeKey((coord, -temp))
            max_h.deleteKey((coord, 0))
            
        elif min_h.isInMinHeap(coord):
            # min_h.changeKey((coord, temp))
    ...
    print(f"Time to execute : {timeit.default_timer() - t1:.2f}")
